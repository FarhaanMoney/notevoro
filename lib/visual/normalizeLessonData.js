...
function normalizeLessonData(raw) {
  ...
  const slides = safeArray(lesson.slides).map((slide, index) => ({
    ...safeObject(slide),
    id: safeString(slide?.id, `slide-${index + 1}`),
    title: safeString(slide?.title, `Slide ${index + 1}`),
    description: safeString(slide?.content, slide?.visualPrompt || ''),
    narration: safeString(slide?.content, slide?.visualPrompt || ''),
    visualCues: safeArray(slide?.bulletPoints).map((cue) => safeString(cue, '')).filter(Boolean),
    actions: [],
  }));
  const steps = safeArray(lesson.steps).map((step, index) => normalizeStep(step, index));

  const mergedSteps = [...slides, ...steps];

  const normalized = {
    lessonType: safeString(lesson.lessonType, 'general').toLowerCase(),
    template: safeString(lesson.template, 'default').toLowerCase(),
    title: safeString(lesson.title || lesson.topic, 'Untitled Lesson'),
    ...
    steps: mergedSteps,
    currentStepIndex: Math.max(0, Math.min(mergedSteps.length - 1, typeof lesson.currentStepIndex === 'number' ? lesson.currentStepIndex : 0)),
    currentStep: null,
  };
...
export { normalizeLessonData, createFallbackLessonData, isLessonData, normalizeStep, safeArray, safeString, safeObject };
