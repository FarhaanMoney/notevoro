...
import { getUserById, createUser, updateUser, getUserByPhone } from './userManager.js';
...
export async function generateLinkingToken(userId) {
  ...
  // If the in-memory user store doesn't have the user (common in prod), try bootstrapping
  if (!user) {
    try {
      const sb = supabaseAdmin();
      const { data: userRow, error } = await sb.from('users').select('id, email, name, phone_number').eq('id', userId).maybeSingle();
      if (error) {
        console.warn('generateLinkingToken: failed to fetch user from DB', { userId, error });
      }
      if (userRow) {
        user = createUser({ id: userRow.id, name: userRow.name || userRow.email || 'User', email: userRow.email || undefined, phone: userRow.phone_number || undefined });
        console.log('generateLinkingToken: bootstrapped legacy user into memory', { userId });
      }
    } catch (err) {
      console.warn('generateLinkingToken: bootstrap attempt failed', { userId, err });
    }
  }
