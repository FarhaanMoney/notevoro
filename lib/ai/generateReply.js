...
function resolveOpenAIBaseUrl() {
  ...
  try {
    ...
    console.warn('⚠️ Invalid OPENAI_BASE_URL, falling back to default:', { raw, error: err.message });
    return DEFAULT_OPENAI_BASE_URL;
  }
}
...
export async function generateReply(userMessage, phoneNumber, conversationHistory = [], userContext = null) {
  try {
    ...
    try {
      ...
      console.warn('⚠️ AI API fetch failed, retrying with default base URL', {
        phoneNumber,
        errorMessage: fetchError.message,
        attemptedUrl: apiUrl,
        fallbackUrl: `${DEFAULT_OPENAI_BASE_URL}/chat/completions`
      });

      if (OPENAI_BASE_URL !== DEFAULT_OPENAI_BASE_URL) {
        response = await fetch(`${DEFAULT_OPENAI_BASE_URL}/chat/completions`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${API_KEY}`,
            'Content-Type': 'application/json'
