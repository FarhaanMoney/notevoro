...
export async function startProTrialDb(userId) {
  ...
  if (updErr) {
    ...
    if (retryErr.includes('trial_used')) {
      ...
      if (retry.error) {
        console.error('Failed to start trial on retry without trial_used', retry.error);
        throw new Error('Failed to start trial');
      }
    } else if (retryErr.includes('trial_ends_at')) {
      delete updates.trial_ends_at;
      const retry = await sb.from('users').update(updates).eq('id', userId);
      if (retry.error) {
        console.error('Failed to start trial on retry without trial_ends_at', retry.error);
        throw new Error('Failed to start trial');
      }
    } else {
